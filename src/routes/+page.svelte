<script lang="ts">
	...
	import { pageState, actions } from './main.svelte';
					...
					<div class="list-group list-group-flush border rounded bg-white overflow-auto style-scroll" style="max-height: 52vh;">
						...
						{#each filteredPlaylists as pl}
							<button 
								...
								class:active-playlist={pageState.selectedPlaylistId === pl.playlist_id}
		...
		<div class="col-12 col-md-8 col-lg-9">
			...
			<div class="card-header bg-light border-bottom py-3 d-flex justify-content-between align-items-center">
				...
				{#if pageState.selectedPlaylistId}
					...
					<button class="btn btn-sm btn-dark fw-medium px-3 d-flex align-items-center gap-2" onclick={() => actions.sendMpcCommand({ load: pageState.selectedPlaylistId! })}>
						<i class="bi bi-play-circle-fill"></i> Воспроизвести весь плейлист
					</button>
				{/if}
					...
					<div class="table-responsive border rounded bg-white style-scroll" style="max-height: 68vh;">
						<table class="table table-light table-hover mb-0 align-middle">
							...
							<tbody>
								{#if pageState.currentTracks.length === 0}
									<tr>
										<td colspan="3" class="text-center text-muted py-5">Плейлист пуст или не выбран</td>
									</tr>
								{/if}
								{#each pageState.currentTracks as tr}
									<tr class="cursor-pointer" onclick={() => actions.sendMpcCommand({ load: pageState.selectedPlaylistId!, track: tr.track_number })}>
